...
function normalizeEntry(raw, previous) {
	...
	const num = (key) => {
		const v = raw[key];
		if (typeof v !== "string" && typeof v !== "number") throw new Error(`price field "${key}" must be a number`);
		const n = Number(v);
		if (v === "" || !Number.isFinite(n) || n < 0) throw new Error(`price field "${key}" must be a non-negative number`);
		return n;
	};
	const entry = {
...
async function scanSessionFile(path, byModel, all, resolvePrice) {
	...
	let model = null;
	let provider = null;
	/** Last usage sample per turn/step (replacement support, any order). */
	...
	for (const line of text.split("\n")) {
		...
		for (const event of events) {
			...
			if (prev !== void 0) {
				...
				// wherever it appeared in the log (retries, tool loops, ...).
				const bucket = ensureBucket(byModel, prev.model, prev.day);
				bucket.input -= prev.added.input;
				bucket.output -= prev.added.output;
				bucket.cacheRead -= prev.added.cacheRead;
				bucket.cacheWrite -= prev.added.cacheWrite;
				bucket.reasoning -= prev.added.reasoning;
				bucket.total -= prev.added.total;
				bucket.cost -= prev.added.cost;
				bucket.costInput -= prev.added.costInput;
				bucket.costOutput -= prev.added.costOutput;
				bucket.costCacheRead -= prev.added.costCacheRead;
				bucket.costCacheWrite -= prev.added.costCacheWrite;
				const allBucket = ensureBucket(all, "__all__", prev.day);
				allBucket.input -= prev.added.input;
				allBucket.output -= prev.added.output;
				allBucket.cacheRead -= prev.added.cacheRead;
				allBucket.cacheWrite -= prev.added.cacheWrite;
				allBucket.reasoning -= prev.added.reasoning;
				allBucket.total -= prev.added.total;
				allBucket.cost -= prev.added.cost;
				allBucket.costInput -= prev.added.costInput;
				allBucket.costOutput -= prev.added.costOutput;
				allBucket.costCacheRead -= prev.added.costCacheRead;
				allBucket.costCacheWrite -= prev.added.costCacheWrite;
			}
...
function apply(ctx, config) {
	...
	const compute = async () => {
		...
		for (const modelName of Object.keys(byModel)) {
			...
			resolved[modelName] = UNKNOWN_PRICE === entry ? { ...entry, source: "estimated" } : entry;
		}
		return {
...
/** Internals exposed for unit tests (pure helpers + the price store). */
const internals = {
	DEEPSEEK_OFF_PEAK,
	UNKNOWN_PRICE,
	DEFAULT_PRICES,
