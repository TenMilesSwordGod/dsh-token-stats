...
.dshtk-tip{position:fixed;z-index:220;pointer-events:none;transform:translate(-50%,-100%);max-width:300px;padding:9px 12px;border:1px solid var(--dsw-alias-border-l2);border-radius:10px;background:var(--dsw-alias-tooltip-bg);color:var(--dsw-alias-label-primary-inverted);box-shadow:0 10px 28px var(--dsw-alias-bg-mask-2);font-size:11px;line-height:1.55;animation:dshtk-tip-in .16s var(--ds-ease-in-out)}
		...
		function PriceModal({ t, open, onClose, modelParts, onChanged }) {
			...
			const save = async () => {
				...
				for (const model of rowModels) {
					...
					if (![input, output, cacheRead, cacheWrite].every((v) => Number.isFinite(v) && v >= 0)) { ok = false; continue; }
					...
					if (d.offPeakOn === true) {
						...
						if (!Number.isFinite(multiplier) || multiplier <= 0 || multiplier > 1 || !Number.isFinite(startUtc) || !Number.isFinite(endUtc) || startUtc < 0 || startUtc > 24 || endUtc <= startUtc || endUtc > 48) { ok = false; continue; }
				...
				try {
					...
					setTable(json.models ?? {});
					setDraft({ ...(json.models ?? {}) });
